...
const BENEFITS = [
  { icon: 'headphones', title: 'Narração profissional e curada', detail: 'Vozes humanas dinâmicas em velocidade até 2x' },
  { icon: 'download_for_offline', title: 'Modo offline completo', detail: 'Baixe audiobooks e leia sem depender de internet' },
  { icon: 'send_to_mobile', title: 'Sincronização com Kindle e PDF', detail: 'Envie resumos estruturados com um simples toque' },
  { icon: 'auto_stories', title: 'Lançamentos globais semanais', detail: 'Best-sellers mundiais traduzidos e dissecados' },
]
...
function PlansPage() {
  ...
  const [selected, setSelected] = useState(data.plans.find((plan) => plan.isFeatured)?.slug ?? data.plans[0]?.slug)
  ...
  async function handleSubscribe() {
    ...
    setSubmitting(true)
    const result = await subscribeToPlan({ data: { planSlug: selectedPlan.slug } })
    setSubmitting(false)
    if (!result.ok) {
      show(result.error.message)
      return
    }
    show(selectedPlan.trialDays > 0 ? 'Teste de 7 dias ativado!' : 'Assinatura mensal ativada!')
        ...
        <div className="flex items-center justify-between">
          ...
          {current ? (
            <span className="rounded-full bg-primary-fixed px-3 py-1 text-[10px] font-bold tracking-wider text-primary uppercase">
              {current.isTrialing ? `Teste: ${current.trialDaysLeft}d` : 'Premium'}
            </span>
          ) : (
      ...
      <div className="flex flex-col px-5 pt-3 pb-8">
        <div className="mb-6 flex flex-col items-center text-center">
          <span className="mb-2 inline-flex items-center gap-1.5 rounded-full bg-secondary-fixed px-3 py-1 text-secondary shadow-sm">
            ...
            <span className="text-[10px] font-bold tracking-wider uppercase">Lúmina Premium</span>
          </span>
          ...
          <p className="max-w-xs text-[15px] leading-relaxed text-on-surface-variant">
            Acesso irrestrito a mais de 2.500 sínteses literárias essenciais em áudio imersivo e
            texto diagramado.
          </p>
        </div>
        ...
        <div className="mb-8 flex flex-col gap-2.5">
          {BENEFITS.map((benefit) => (
            <div key={benefit.title} className="flex items-center gap-3 rounded-xl bg-surface-container-lowest p-3.5 shadow-sm">
              <span className="grid h-10 w-10 shrink-0 place-items-center rounded-lg bg-primary/10 text-primary">
                <Icon name={benefit.icon} filled className="text-[22px]" />
              </span>
              <div className="min-w-0">
                <span className="block text-[15px] leading-tight text-on-surface">{benefit.title}</span>
                <span className="block truncate text-[13px] text-on-surface-variant">{benefit.detail}</span>
              </div>
              <Icon name="check_circle" filled className="ml-auto shrink-0 text-[20px] text-secondary" />
            </div>
          ))}
        </div>
              ...
              >
                ...
                <div className="mb-1.5 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[16px] font-semibold text-on-surface">{plan.name}</span>
                  </div>
                  {plan.trialDays > 0 ? (
                    ...
                    <span className="text-[11px] text-on-surface-variant">Flexível</span>
                  )}
                </div>
                <div className="mt-2 flex items-baseline justify-between border-t border-surface-container-highest pt-2">
                  <div className="flex flex-col">
                    <div className="flex items-baseline gap-1">
                      <span className={`text-[24px] leading-none font-bold ${active ? 'text-primary' : 'text-on-surface'}`}>
                        {formatCurrencyBRL(plan.monthlyEquivalentCents)}
                      </span>
                      <span className="text-[13px] text-on-surface-variant">/ mês</span>
                    </div>
                  ...
                  <span className={`text-[11px] font-semibold ${active ? 'text-secondary' : 'text-outline'}`}>
                    {plan.interval === 'yearly' ? 'Melhor custo-benefício' : 'Cancele quando quiser'}
                  </span>
                </div>
              </button>
            )
        ...
        <div className="mb-8 flex flex-col rounded-xl bg-surface-container-low p-4">
          <div className="mb-3 flex items-center gap-2">
            ...
            <span className="text-[13px] text-on-surface-variant">(+28 mil leitores)</span>
          </div>
          <p className="font-serif mb-3 text-[17px] leading-snug text-on-surface italic">
            “O Lúmina transformou meu trajeto matinal em uma pós-graduação constante. É a ferramenta
            mais eficiente que uso hoje.”
          </p>
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[12px] text-on-surface-variant">Head de Produto & Inovação</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center gap-3">
          {current ? (
            <>
              <div className="flex w-full items-center justify-between rounded-xl bg-primary-fixed/40 p-4">
                <div>
                  <p className="text-[13px] font-semibold text-primary">Assinatura ativa</p>
                  <p className="text-[12px] text-on-surface-variant">
                    {current.plan.name} •{' '}
                    {new Date(current.subscription.currentPeriodEnd).toLocaleDateString('pt-BR')}
                  </p>
                </div>
                <button
                  type="button"
          ...
          <div className="mt-1 flex flex-col items-center gap-1.5 text-center">
            <div className="flex items-center gap-1.5 text-[12px] text-on-surface-variant">
              <Icon name="verified_user" className="text-[16px] text-surface-tint" />
              <span>Sem cobrança imediata. Cancele facilmente com 1 clique.</span>
            </div>
